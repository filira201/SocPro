...
import { useProjectsFeedSkillsCatalog } from "../lib/use-projects-feed-skills-catalog";
...
type ProjectsFeedSkillsFilterProps = {
  skillIds: string[];
  onSkillIdsChange: (ids: string[]) => void;
  disabled?: boolean;
};

export function ProjectsFeedSkillsFilter({
  skillIds,
  onSkillIdsChange,
  disabled,
}: ProjectsFeedSkillsFilterProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <p className="text-muted-foreground text-sm">
        Навыки: только из каталога. Показываются проекты, у которых в
        требованиях есть <span className="text-foreground">каждый</span> из
        выбранных навыков (если добавить навык, которого нет в требованиях
        проекта, он исчезнет из выдачи).
      </p>
